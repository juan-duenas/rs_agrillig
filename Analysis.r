...
adonis2(otu_table(f)~f_tr$dl, permutations = 9999, method = 'jac', binary=T, by="margin")
...
# Figure 1 sup ------ multiple factor effects ####
...
rs <- read_delim("rs_db.csv", delim = ",") %>% 
       ...
       names_to = "var",
       values_to = "y")%>%
       select(potID, dl, whc, treatment, no_poll, var, y)%>%
       rename(x=dl, z=whc, p=no_poll, t=treatment)%>%
       filter(!is.na(y))%>%
       group_split(var)%>% # split into several databases by treatment and store on a list
       set_names(., nm = c("cf", "mwd", "imp", "wsa"))%>% #give names to each set  
       as.list(.)
...
# alpha diversity
alf <- cbind(sample_data(f)[,c(1)], estimate_richness(f, measures=c("InvSimpson")))%>%
       mutate(var=rep("invsimp",255))%>%
       right_join(.,rs[["cf"]], by="potID")%>% #fixed 
       select(-c(y, var.y))%>%
       rename(var=var.x, y=InvSimpson)%>%
       select(1,4,5,6,7,3,2) %>%
       filter(!is.na(y)) %>% as_tibble(.) %>%
       mutate_at('potID', as.integer) 
...
Mymodselc <- function(df, fml) {
  # function to set two pairs of models one linear one gam  
  gms <- list(
    gm1 = gam(data = df, formula = y~ t + s(x, t, bs = 'fs', k=3), family = fml, method="ML", select = T),
    gm2 = gam(data = df, formula = y~ s(x, t, bs = 'fs', k=3), family = fml, method="ML", select = T)
  )
}
...
Mygamsc=function(df, fml){
  ...
  # returns a list of named gams.
  # argument fml is a string list stating the family of the conditional prob. and must be provided separately
  md=list(gm=gam(data = df, formula = y~ t + s(x, t, bs = 'fs', k=3), family = fml, method="REML", select = T))
}
...
write_csv(sm, "table3") # good old way to save results
# check model fit with diagnostics 
#overview(mds$alf.mod)
check <- function(b, k.sample = 5000, k.rep = 200) {
  mgcv:::k.check(b, subsample = k.sample, n.rep = k.rep)
...
# Permanovas
# Multivariate Model (PERMANOVA)
adonis2(otu_table(f)~f_tr$t, permutations = 999, method = 'jac', binary=T, by="term")
...
Myplotsc=function(df, fml, laby, se){
  ...
  colrs <- RColorBrewer::brewer.pal(6, "Set2")
  plot=list(ggplot(data=df, aes(x=x, y=y))+
              geom_point(alpha=0.05, color="Blue")+
              # geom_smooth(data=df, aes(x=x, y=y), method = "gam", formula = y ~ s(x, bs="tp", k=3), 
              #        method.args=list(family=fml),
              #        se=T, color="Black")+
              geom_line(data=se, aes(x=x, y=fit, color=t), linewidth=1.5, inherit.aes = F)+ # use fit estimated by predict
              #geom_ribbon(data=se,                                  # estimate alpha=95% Credible Intervals
              #            aes(x=x, ymin = fit- 1.96*(se.fit), ymax = fit + 1.96*(se.fit)), 
              #            alpha = 0.5, fill = "Grey", inherit.aes = F)+ # this constant comes from qnorm((1-0.95)/2, lower.tail = F)
              scale_x_continuous(breaks = c(1,3,5,7,9,11,13,15))+
              scale_color_manual(values=colrs)+
              labs(y=laby, x="Dryness level", colour="Pollutant ID")+
              theme(text = element_text(size = 12),
                    panel.grid.minor = element_blank()))
}
...
f_tr <- sample_data(f)[,c(1,2)] %>% data.frame(.) %>% rename(t=treatment) %>%
...
rm(betf)
ford <- ford %>% filter(x<1) %>% filter(y<1)
...
ord <- ggplot() + 
  geom_vline(xintercept=0.0, color="White", linewidth=1, linetype=1)+
  geom_hline(yintercept=0.0, color="White", linewidth=1, linetype=1)+    
  geom_point(data=ford, aes(x=x, y=y, color=t), alpha=0.8,  size=2, show.legend = T) +
  scale_color_manual(values=colrs)+
  labs(x="Axis 1", y="Axis 2", color="Pollutant ID")+
  theme(axis.title = element_text(size = 12),
        legend.key = element_blank(),  #removes the box around each legend item
        legend.position = "right", #legend at the bottom
        legend.text = element_text(size=12),
...
(figs1 <- ggarrange(pl$mwd, pl$wsa, pl$imp, pl$cf, pl$alf, pl$ord, ncol = 3, nrow = 2,
                   common.legend = T, legend = "right", labels = c('a', 'b', 'c', 'd', 'e', 'f')))
